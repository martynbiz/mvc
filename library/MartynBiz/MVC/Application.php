...
class Application
{
    ...
    */
    public function __construct($config=array(), $environment=array())
    {
        ...
        // because getallheaders doesn't exist within CLI, we need to check for testing
        if(is_callable('getallheaders')) {
            $this->environment( getallheaders() );
        }
        $this->environment( $_SERVER );
        $this->environment('POST', $_POST);
        $this->environment($environment); // this will overwrite any environments set
        
        //handle PUT and DELETEs
        $method = strtoupper($this->environment('REQUEST_METHOD'));
        if($method == 'POST') {
            if(isset($this->environment['POST']['_METHOD'])) {
                $method = strtoupper($this->environment['POST']['_METHOD']);
            }
        }
    ...
    */
    protected function setResource(&$resource, $config, $value=null)
    {
        if (is_array($config)) { // multiple set
            ...
            // merge with array
            $resource = array_merge($resource, $config);
            ...
            if(isset($resource[$config])) {
                ...
                if(is_callable($single)) {
                    $resource[$config] = $single();
                }
        ...
        // // return all - why can't we return all?
        // return $this->config;
    }
    ...
    */
    public function isAjax()
    {
        ...
        // ** actually we should fetch this from app encase we are mocking the environment
    ...
    */
    public function run($environment=array())
    {
        ...
        $url = (! is_null($this->environment('PATH_INFO'))) ? 
            $this->environment('PATH_INFO') : 
            $this->environment('REQUEST_URI');
        ...
        if(! $controller)
            throw new \Exception('Controller \'' . $route['controller'] . '\' not found in services.');
